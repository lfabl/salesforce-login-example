...
const Login = () => {
    ...
    useEffect(() => {
        ...
        const urlWithParams = `${AUTHORIZATION_URL}?${params}`;
    ...
    const login = () => {
        ...
        const urlWithParams = `${TOKEN_URL}?password=${password}&${params}`;
        ...
        fetch(urlWithParams, {
            ...
            .then((res) => {
                ...
                navigation.navigate("authorization", {
                    url: urlWithParams,
                    getAuthorize: () => {
                        const isAuthorize = getAuthorize(urlWithParams);
                        if(isAuthorize) {
    ...
    const getAuthorize = (urlWithParams: string) => {
        ...
        fetch(urlWithParams, {
            ...
            .then((res) => {
                if(res.error) {
                    Alert.alert("Authorization error", res.error_description);
                    return;
                }

                isAuthorize = true;
                Toast.show('Successfully authorized.', Toast.SHORT);
            })
            .catch((err) => {
                isAuthorize = false;
                Alert.alert("An error occurred", err.message);
